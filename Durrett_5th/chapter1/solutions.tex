...
\begin{solution}
    ...
    Thus
    \[
        Y_n = \sum_{m=-n2^n}^{n2^n-1}  \frac{m}{2^n} 1_{A_{m,n}}
    \]
    where $A_{m,n}=Y^{-1}([\frac{m}{2^n},\frac{m+1}{2^n}))\in \sigma(X)$ are disjoint. And since $\sigma(X) = X^{-1}(\mathcal{B})$,
    there must be $C\in \mathcal{B}$ such that $X^{-1}(C)=A_{m,n}$.
    Let
    \[
        B_{m,n}=\bigcap_{C_i\in \mathcal{C}} C_i,\quad \mathcal{C} = \Dset{X\in \mathcal{B}}{X^{-1}(C)=A_{m,n}}
    \]
    and then $B_{m,n}\in \mathcal{B}$, $X^{-1}(B_{m,n})=A_{m,n}$ and $B_{m,n}$ are disjoint for a fixing $n$ and different $m$.
    So we define
    \[
        f_n = \sum_{m=-n2^n}^{n2^n-1}  \frac{m}{2^n} 1_{B_{m,n}}
    \]
...
\end{exercise}
\begin{solution}
    Just a hint for \cref{exec: 1.3.8}. The specific forms of simple functions approaching to $Y$ is not critical.
    But it's curious that if the diction here implies there is a simpler and non-constructive proof.
\end{solution}

\section{Integration}
\begin{exercise}
    Show that if $f \ge 0$ and $\int f \dif \mu = 0$ then $f = 0$ a.e.
\end{exercise}
\begin{solution}
    Let $A_n={f\ge \frac{1}{n}}$.
    Then
    \[
        0=\int f \dif \mu \ge \int \frac{1_{A_n}}{n} \dif \mu= \frac{1}{n}\mu(A_n) \ge 0
    \]
    Hence $\mu(A_n)=0$ and $\mu(A_n^c)=1$. Thus $\mu(\Set{f=0}) = \mu(\bigcap A_n^c) =1$
\end{solution}

\begin{exercise}
    Let $f \ge 0$ and $E_{n,m} = \Set{x : m/2^n \le f(x) < (m + 1)/2^n}$. As
    $n \to \infty$,
    \[
...
\end{exercise}
\begin{solution}
    Let $f_n=  \sum_{m=1}^\infty \frac{m}{2^n}1_{E_{n,m}}$. And then left hand is equal to $\int f_n\dif \mu$ and $\int f_n\dif \mu \le \int f\dif \mu$.
    Recall
    \[
        \int f \dif \mu = \sup \Set{\int h \dif \mu : 0 \le h \le f, h \text{ is bounded and } \mu(\Set{x : h(x) > 0}) < 1}
    \]
    For such $h$ and $C=\Set{x: h(x)>0}$, $h\le 1_C\sum_{m=1}^\infty \frac{m+1}{2^n}1_{E_{n,m}} \le 1_C(f_n+\frac{1}{2^n})$.
    So
    \[
        \int f_n \dif \mu \ge \int h \dif \mu - \frac{\mu(C)}{2^n}\to \int h \dif \mu
    \]
    The desired result follows from
    \[
        \int f \dif \mu \ge \limsup \int f\dif\mu \ge \liminf \int f\dif\mu \ge \sup_h \Set{\int h\dif \mu} = \int f\dif \mu
    \]


\end{solution}
\begin{exercise}
    Let $g$ be an integrable function on $\RR$ and $\epsilon > 0$.
    \begin{enumerate}[label=(\roman*)]
        \item Use the definition of the integral to conclude there is a simple function $\varphi =\sum_k b_k1_{A_k}$ with $\int \abs{g-\varphi} \dif x<\epsilon$.
        \item Use Lemma A.2.1 to approximate the $A_k$ by finite unions of intervals to get a step function
              \[
                  q=\sum_{j=1}^{k} c_j1(a_{j-1},a_j)
              \]
              with $a_0 < a_1 < \dots < a_k$, so that $\int \abs{\varphi-q} \dif x <\epsilon$.
        \item Round the corners of $q$ to get a continuous function $r$ so that $\int\abs{q-r}\dif x < \epsilon$.
...
\begin{solution}
    \leavevmode
    \begin{enumerate}[label=(\roman*)]
        \item Let $g=g^+ - g^-$ where $g^+ = g\lor 0$ and $g^- =-g\lor 0$. Thus there are simple functions $h_1, h_2$ such that
              $\int \abs{g^+ -h_1} \dif x<\flatfrac{\epsilon}2$ and $\int \abs{g^- -h_2} \dif x<\flatfrac{\epsilon}2$. Then $\varphi = h_1-h_2$ is a simple function and
              \[
                \int \abs{g -\varphi} \dif x = \int \abs{(g^+-h_1) - (g^- -h_2)} \dif x \le \int \abs{g^+ -h_1} \dif x + \int \abs{g^- -h_2} \dif x < \epsilon
              \] 

        \item To make a continuous function replace each $c_j1_{(aj-1,aj )}$ by  a function that is 0 \textbf{on} $(a_{j-1}, a_j)^c$, $c_j$ on $(a_{j-1+\delta}, a_j-\delta)$, and linear otherwise. If
    \end{enumerate}
